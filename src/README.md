...
- `CCTVSim.Application`: CLI/Config/Progress 처리
- `CCTVSim.Orchestration`: Session/Scenario/Pipeline 조정
- `CCTVSim.Simulation`: Unity Adapter 및 IPC
- `CCTVSim.DataPipeline`: Worker/Queue 구현
- `CCTVSim.Services`: Validation/Stats/Manifest

### 초기화 체크리스트

자동 스크립트: `src/init_sln.sh` (dotnet SDK 필요)

1. `./init_sln.sh` 실행 → `CCTVSim.sln`과 Class Library 프로젝트 자동 생성
2. `CCTVSim.Application` 프로젝트에 CLI 엔트리포인트(`Program.cs`) 생성
